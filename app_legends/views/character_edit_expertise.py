...
from app_legends.models import Character, CharacterExpertise, CharacterAttribute, CharacterStats, CharacterWeapon, Weapon
...
class CharacterEditExpertise(View):
    def post(self, request, id):
        ...
        characterWeapons = CharacterWeapon.objects.filter(character=character).order_by('-id')
        ...
        if expertise and attribute:
            ...
            character.save()
            characterStats.save()
            for weapon in characterWeapons:
                weapon.save()
